...
{
    public partial class Salaries2 : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        #region Variables
        ...
        protected void BtnExportGrid_Dtl_Click(object sender, EventArgs args)
        {
            try
            {
                ...
                if (!rbView.Checked)
                {
                    ...
                    catch (Exception ex) { AlertMsg.MsgBox(Page, ex.Message); }
                }
                else
                {
                    SqlDataReader dr = Leaves.GetEmployeesForSalariesExportToExcel(SiteID, DeptID, Month, Year, Status, EmpNat);//, Convert.ToInt32(ViewState["NoOfRecords"]));
        ...
        protected void gvPaySlip_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Run")
            {
                ...
                AttendanceDAC.HR_SavePaySLIP2(EmpId, Date, 0, "Salary");
            ...
            try
            {
                ...
                if (txtEmpID.Text != "")
                {
                    ...
                    try
                    {
                        ...
                        DataSet dsConfig = AttendanceDAC.CheckEmployeeConfiguration(EmpID);
                        if (dsConfig.Tables[0].Rows.Count > 0)
                        {

                            if (EmpID != 0)
                            {
                                AttendanceDAC.HR_SavePaySLIP2(EmpID, st, valofSync, "Salary");
                                btnSearch_Click(sender, e);
                                if (valofSync == 0)
                            ...
                            AlertMsg.MsgBox(Page, "Configuration not done!");
                ...
                else
                {
                    ...
                    if (SiteID != 0)
                    {
                        ...
                        foreach (DataRow dr in ds.Tables[0].Rows)
                        {
                            try
                            {
                                ...
                                DataSet dsConfig = AttendanceDAC.CheckEmployeeConfiguration(empid);
                                  if (dsConfig.Tables[0].Rows.Count > 0)
                                  {
                                      AttendanceDAC.HR_SavePaySLIP2(Convert.ToInt32(dr["EmpID"]), st, valofSync, "Salary");
                                  }
                                  else
                                      empids = empids + empid+",";
                            }
                        ...
                        if (empids == string.Empty)
                        {
                            ...
                            AlertMsg.MsgBox(Page, empids +"For these employees configuration not done!");
                    }
                    else
