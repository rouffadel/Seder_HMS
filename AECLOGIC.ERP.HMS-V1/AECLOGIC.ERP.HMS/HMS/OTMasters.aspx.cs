...
{
    public partial class OTMasters : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        public void BindGrid()
        {
            ...
            DataSet ds = PayRollMgr.GetOTDetails(0);
        ...
        public void BindDetails(int ID)
        {
            ...
            tblNew.Visible = true;
            pnltblNew.Visible = true;
            DataSet ds = PayRollMgr.GetOTDetails(ID);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                int OTID = 0;
                if (ViewState["OTID"].ToString() != null && ViewState["OTID"].ToString() != string.Empty)
                {
                    OTID = Convert.ToInt32(ViewState["OTID"].ToString());
                }
                int CompanyID = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["CompanyID"].ToString());

                int Output = PayRollMgr.HR_OTNamesAddEdit(OTID, txtName.Text.Trim(), txtSName.Text, CompanyID);
                if (Output == 1)
