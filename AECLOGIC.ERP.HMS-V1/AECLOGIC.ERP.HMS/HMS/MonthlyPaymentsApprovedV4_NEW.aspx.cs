...
{
    public partial class MonthlyPaymentsApprovedV4_NEW : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        protected void btnExcelExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (Convert.ToInt32(ddlFormat.SelectedValue) == 1)
                {
                    ...
                    char Status = 'n';
                    SqlDataReader dr = Leaves.GetEmployeesForSalariesExportToExcel((Convert.ToInt32(ddlworksite.SelectedValue)), 0, Convert.ToInt32(Month), Convert.ToInt32(Year), Status, 1);//, Convert.ToInt32(ViewState["NoOfRecords"]));
                    ...
                    Response.Redirect("MonthlyPaymentsApprovedV4_NEW.aspx", false);
                }
        ...
        public static DataSet HR_GetEMPSalariesExporttoExcel_Rev4(DateTime Date, int? EMPID, int? WSID, int? Dept)
        {
            ...
            return SQLDBUtil.ExecuteDataset("[HR_GetEMPSalariesExporttoExcel_Rev4_CashNew]", sqlParams);
        }
        private void ExcelExport3(HRCommon objHrCommon)
        {
            ...
            {
                clsErrorLog.HMSEventLog(ex, "Salaries", "ExcelExport3", "006");
            }
        }
        private void ExcelExport2(HRCommon objHrCommon)
        {
            ...
            {
                clsErrorLog.HMSEventLog(ex, "Salaries", "ExcelExport2", "006");
            }
        }
        private void releaseObject(object obj)
        {
            ...
            {
                obj = null;
                AlertMsg.MsgBox(Page, "Exception Occured while releasing object " + ex.ToString());
            }
            finally
            {
                GC.Collect();
        ...
        private DataSet USP_Salaries_All_Approved_43(int? SiteID, int? Month, int Year)//, int PageSize)
        {
            try
            {
                ...
                DataSet ds = SQLDBUtil.ExecuteDataset("USP_Salaries_Approved_Rev4_Cash", sqlParams);
                return ds;
            }
            catch (Exception e)
            {
                clsErrorLog.HMSEventLog(e, "Salaries", "USP_Salaries_All_Approved_43", "006");
                throw e;
            }
        }
        private DataSet USP_Salaries_All_Approved_42(int? SiteID, int? Month, int Year)//, int PageSize)
        {
            try
            {
                ...
                sqlParams[5] = new SqlParameter("@Month", Month);
                sqlParams[6] = new SqlParameter("@Year", Year);
                DataSet ds = SQLDBUtil.ExecuteDataset("USP_Salaries_Approved_Rev42_Cash", sqlParams);
                return ds;
            }
            catch (Exception e)
            {
                clsErrorLog.HMSEventLog(e, "Salaries", "USP_Salaries_All_Approved_42", "006");
                throw e;
            }
        }
        private DataSet USP_Salaries_All_Approved_Search_4(int? SiteID, int? Month, int Year, int Empid)//, int PageSize)
        {
            ...
            {
                clsErrorLog.HMSEventLog(e, "Salaries", "USP_Salaries_All_Approved_Search_4", "006");
                throw e;
            }
        }
