...
{
    public partial class ConfigOffenses : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        void BindGrid(HRCommon objHrCommon)
        {
            ...
            try
            {
                ...
                // added newly.. 14-03-2016
                DataSet ds = PayRollMgr.GetEmpPenaltiesItemsListByPaging(objHrCommon);
                if (ds != null && ds.Tables.Count != 0 && ds.Tables[0].Rows.Count > 0)
                {
        ...
        public void BindDetails(int ID)
        {
            ...
            pnltblNew.Visible = true;
            //14-03-2016
          DataSet  ds = PayRollMgr.GetEmpPenaltiesDetails(ID);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
        ...
        protected void gvWages_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else
            {
                try
                {
                    // Added by Rijwan : 07-06-2016
                    SqlParameter[] p = new SqlParameter[1];
                    p[0] = new SqlParameter("@RMItemId", ID);
                    SQLDBUtil.ExecuteNonQuery("Del_EmpPenalties", p);
                    BindGrid(objHrCommon);
                }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int OutPut = PayRollMgr.InsUpdEmpPenalties(WagesID, txtName.Text.Trim(), CompanyID);
                if (OutPut == 1)
