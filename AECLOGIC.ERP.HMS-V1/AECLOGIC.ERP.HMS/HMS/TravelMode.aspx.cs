...
{
    public partial class TravelModeV1 : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                parms[2] = new SqlParameter("@Name", txtName.Text.Trim());
                parms[3] = new SqlParameter("ReturnValue", System.Data.SqlDbType.Int); parms[3].Direction = ParameterDirection.ReturnValue;
                int Output = SqlHelper.ExecuteNonQuery("SP_th_Travelmode_Insert_Update_Delete_VIEW_Paging_Select", parms);
        ...
        public void clear() { hdID.Value = "0"; txtName.Text = ""; }
        ...
        void GridBind(HRCommon objHrCommon)
        {
            try
            {
                ...
                parms[4].Direction = ParameterDirection.ReturnValue; int hdName = 0; if (txtSName.Text.Trim() != "") { hdName = Convert.ToInt32(hdSName.Value == "" ? "0" : hdSName.Value); }
                if (hdName != 0)
                {
                    parms[6] = new SqlParameter("@Name", txtName.Text);
                }
                else
                {
                    parms[6] = new SqlParameter("@Name", SqlDbType.Int);
                }

                DataSet ds = SqlHelper.ExecuteDataset("SP_th_Travelmode_Insert_Update_Delete_VIEW_Paging_Select", parms);
        ...
        public void BindDetails(int ID)
        {
            try
            {
                ...
                parms[0] = new SqlParameter("@ID", ID);
                parms[1] = new SqlParameter("@Name", 1);
                parms[2] = new SqlParameter("@fCase", 5);
                DataSet ds = SqlHelper.ExecuteDataset("SP_th_Travelmode_Insert_Update_Delete_VIEW_Paging_Select", parms);
                if (ds != null && ds.Tables.Count != 0 && ds.Tables[0].Rows.Count > 0)
                {
                    EditView.Visible = false; NewView.Visible = true; hdID.Value = ds.Tables[0].Rows[0]["ID"].ToString(); txtName.Text = ds.Tables[0].Rows[0]["Name"].ToString();
                }
                else { }
            }
