...
{
    public partial class MonthlyPaymentAccountPostingV4V1 : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        void EmployeBind(HRCommon objHrCommon)
        {
            try
            {
                ...
                Session["gvPaySlipdDT"] = dsBind.Tables[0];
        ...
        protected void btnExpExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (Session["gvPaySlipdDT"] != null)
                {
                    string strFileName = "EmpSal-" + ddlMonth.SelectedValue + "-" + ddlYear.SelectedValue + "-" + ddlworksite.SelectedItem.Text.ToString();
                    DataTable dtSalEmp = (DataTable)Session["gvPaySlipdDT"];//FetchExportToExcelData((DataTable)Session["gvRecosiledDT"]);
                                                                            //dtExpact = (DataTable)Session["gvRecosiledDT"];
                    ExportFileUtil.ExportToExcel(dtSalEmp, strFileName);
                }
                else
                {
        ...
        //    }
        //}
        protected void btnAccountPosting_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    stDate = 01 + "/" + ddlMonth.SelectedValue + "/" + ddlYear.SelectedValue;
                    int lastdayinmonth = DateTime.DaysInMonth(Convert.ToInt32(ddlYear.SelectedValue), Convert.ToInt32(ddlMonth.SelectedValue));
                    enddate = lastdayinmonth + "/" + ddlMonth.SelectedValue + "/" + ddlYear.SelectedValue;
             ...
             DataSet ds=       USP_Salaries_All_Save4(startdate, EndDate, "");
        ...
        private DataSet USP_Salaries_All_Save4(DateTime startdate, DateTime enddate, string Empid)
        {
            ...
            try
            {
                ...
                sqlParams[0] = new SqlParameter("@paystartdate", startdate);
                sqlParams[1] = new SqlParameter("@payenddate", enddate);
                sqlParams[2] = new SqlParameter("@WSID", ddlworksite.SelectedValue);
                sqlParams[3] = new SqlParameter("@createdby", Convert.ToInt32(Session["UserId"]));

                ds= SQLDBUtil.ExecuteDataset("Monthwise_Salaries_AccountPosting_Rev4", sqlParams);
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count>0)
                {
                    int tbcnt=ds.Tables.Count;
                    AlertMsg.MsgBox(Page, "Account posted Successfully."+ds.Tables[tbcnt-1].Rows[0][0].ToString(), AlertMsg.MessageType.Success);
        ...
        protected void ddlMonth_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            sqlParams[0] = new SqlParameter("@year", ddlYear.SelectedValue);
            sqlParams[1] = new SqlParameter("@fcase", 3);
        ...
        protected void btnNoSalaryEmployees_Click(object sender, EventArgs e)
        {
            ...
            else if (Convert.ToInt32(ddlworksite.SelectedValue) == 0)
            {
                AlertMsg.MsgBox(Page, "Select WorkSite.", AlertMsg.MessageType.Warning);
            }
            else
            {
                Response.Redirect("NoSalaryEmployees.aspx?Month=" + Convert.ToInt32(ddlMonth.SelectedValue) + "&Year=" + Convert.ToInt32(ddlYear.SelectedValue) + "&Wsid=" + Convert.ToInt32(ddlworksite.SelectedValue));
            }
        }
