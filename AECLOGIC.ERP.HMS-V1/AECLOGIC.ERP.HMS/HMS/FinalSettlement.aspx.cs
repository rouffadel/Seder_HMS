...
{
    public partial class FinalSettlement : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        public DataView BindTransdetails(string TransId)
        {
            if (txtempid1.Text!=string.Empty)
            {
               ...
               DataSet ds = PayRollMgr.GetPaySLIP(Convert.ToInt32(TransId), edt);
                double salamount = 0;
                string Details = "";
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[5].Rows.Count > 0)
                {
        ...
        public void BindGridGratuity(HRCommon objcommon, int WSID)
        {
            try
            {
                ...
                int accpost = 1;
                DataTable datatable2 = new DataTable();
                DataSet ds = AttendanceDAC.GetGratuityDetails_Final(objcommon, CompanyID, Convert.ToInt32(ddlworksite.SelectedValue), txtempid1.Text.Trim(), accpost, 1, Convert.ToDateTime("05/13/2016"));
