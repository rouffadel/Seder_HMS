    ...
    public partial class EmpAdvancesV1 : AECLOGIC.ERP.COMMON.WebFormMaster
    {
        ...
        public void InsertAdvance(int Month, int Year)
        {
            try
            {
                ...
                int LoanCategory= Convert.ToInt32(ddlLoanCategory.SelectedValue);
                ...
                if (chkDirectLoan.Checked)
                    IsApproved = 1;
                int LoanIDNo = Convert.ToInt32(HR_InsUpLoansReturnLoanID_AmntReqOnAprV1(LoanID, EmpID, LoanAmount, LoanType, LoanIssuedDate, RecoverYear, RecoverMonth, NoofEMIs, InstallmentAmt, InterestRate, TransID, RecoveryType, IssuedBy, ServiceTax, InstallmentType, TransTime, LoanRemarks, ext, IsApproved, LoanCategory));
        ...
        private int HR_InsUpLoansReturnLoanID_AmntReqOnAprV1(int LoanID, int EmpID, double LoanAmount, int LoanType, DateTime LoanIssuedDate, int RecoverYear, int RecoverMonth, int NoofEMIs, double InstallmentAmt, double InterestRate, int TransID, int RecoveryType, int IssuedBy, double ServiceTax, int InstallmentType, DateTime AmountReqOn, string loanRemarks, string ext,int IsApproved,int LoanCategory)
        {
            try
            {
                ...
                objParam[19] = new SqlParameter("@IsApproved", IsApproved);
                objParam[20] = new SqlParameter("@LoanCategory", LoanCategory);
                SQLDBUtil.ExecuteNonQuery("HR_InsUpLoansReturnLoanID_AmntReqOnV1", objParam);
